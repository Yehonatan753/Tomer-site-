...
export default function LeadMagnetPopup() {
  ...
  useEffect(() => {
    // Show popup after 10 seconds or when user scrolls down 50%
    const timer = setTimeout(() => {
      if (!hasSubmitted && !sessionStorage.getItem('leadMagnetShown')) {
        setIsOpen(true);
            ...
            <div className="p-8">
              {!hasSubmitted ? (
                <>
                  <div className="text-center mb-6">
                    <h3 className="text-2xl font-heading font-bold text-white mb-2">
                      המדריך המלא ליציאה ממעגל הדיאטות
                    </h3>
                    <p className="text-text-muted text-sm">
                      קבל בחינם את השיטה שעזרה למאות מתאמנים להגיע לתוצאות ולשמור עליהן לתמיד, בלי לסבול.
                    </p>
                  </div>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    >
                      <Download size={20} />
                      <span>שלח לי את המדריך עכשיו</span>
                    </button>
                ...
                >
                  ...
                  <h3 className="text-2xl font-bold text-white mb-2">מעולה!</h3>
                  <p className="text-text-muted">
                    המדריך בדרך למייל שלך. בדוק גם בתיקיית קידומי מכירות או ספאם.
                  </p>
                </motion.div>
              )}
            </div>
